  ...
  return (
    ...
    >
      <Select
        ...
        options={options}
        onChange={(value: string) => {
          setSelectedValue(value as IdeType)
          handleSelectIDE(value)
        }}
      />
    </Dialog>
...
function IDECommandFlow({
  ...
  const ideClient = useAppState((s: AppState) =>
    s.mcp.clients.find((c: { name?: string }) => c.name === 'ide'),
  )
          ...
          setAppState(prev => ({
            ...
            mcp: {
              ...
              clients: prev.mcp.clients.filter(
                (c: AppState['mcp']['clients'][number]) => c.name !== 'ide',
              ),
              tools: prev.mcp.tools.filter(
                (t: AppState['mcp']['tools'][number]) =>
                  !t.name?.startsWith('mcp__ide__'),
              ),
              commands: prev.mcp.commands.filter(
                (c: AppState['mcp']['commands'][number]) =>
                  !c.name?.startsWith('mcp__ide__'),
              ),
            },
          }))
        }
