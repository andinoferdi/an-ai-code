...
function getMarketplaceName(): string {
  ...
  return appVariant === 'ant' ? INTERNAL_MARKETPLACE_NAME : OFFICIAL_MARKETPLACE_NAME;
}
function getMarketplaceRepo(): string {
  const appVariant: string = "external";
  return appVariant === 'ant' ? INTERNAL_MARKETPLACE_REPO : OFFICIAL_MARKETPLACE_REPO;
}
...
function ThinkbackFlow(t0: {
  ...
  let t7;
  if ($[12] !== onDone) {
    t7 = function handleAction(action: Exclude<MenuAction, "play">) {
      const prompts: Record<Exclude<MenuAction, "play">, string> = {
        edit: EDIT_PROMPT,
        fix: FIX_PROMPT,
