...
function BridgeToggle({ onDone, name }: Props): React.ReactNode {
  ...
  const replBridgeConnected: boolean = useAppState(s => s.replBridgeConnected)
  const replBridgeEnabled: boolean = useAppState(s => s.replBridgeEnabled)
  const replBridgeOutboundOnly: boolean = useAppState(
    s => s.replBridgeOutboundOnly,
  )
  ...
  useEffect(() => {
    // If already connected or enabled in full bidirectional mode, don't run
    // connect flow here; render the disconnect dialog instead.
...
function BridgeDisconnectDialog({ onDone }: Props): React.ReactNode {
  ...
  const setAppState = useSetAppState()
  const sessionUrl: string | undefined = useAppState(s => s.replBridgeSessionUrl)
  const connectUrl: string | undefined = useAppState(
    s => s.replBridgeConnectUrl,
  )
  const sessionActive: boolean = useAppState(s => s.replBridgeSessionActive)
  ...
  const qrLines = showQR && qrText ? qrText.split('\n').filter(l => l.length > 0) : []
...
async function checkBridgePrerequisites(): Promise<string | null> {
  ...
  if (feature('KAIROS') && useV2) {
    try {
      ...
      const assistantModule = require('../../assistant/index.js') as {
      ...
      // Assistant module is optional in this workspace snapshot.
    }
  }
