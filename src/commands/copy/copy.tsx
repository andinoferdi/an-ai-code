...
function extractCodeBlocks(markdown: string): CodeBlock[] {
  const source = stripPromptXMLTags(markdown)
  const blocks: CodeBlock[] = []
  const codeFenceRegex = /```([^\n`]*)\n([\s\S]*?)```/g
  let match: RegExpExecArray | null = null

  while (true) {
    match = codeFenceRegex.exec(source)
    if (!match) {
      break
    }
    blocks.push({
...
function hasTypeField(block: unknown): block is { readonly type: string } {
...
export function collectRecentAssistantTexts(messages: Message[]): string[] {
  ...
  for (
    ...
    const typedContent = content.filter(hasTypeField)
    if (typedContent.length === 0) continue
    const text = extractTextContent(typedContent, '\n\n')
    ...
    <Pane>
      ...
      >
        ...
        <Select
          ...
          onChange={(selected: PickerSelection) => {
