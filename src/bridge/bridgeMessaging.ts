...
type SDKMessage = SDKUserMessage | SDKGenericMessage
...
export function extractTitleText(m: Message): string | undefined {
  if (m.type !== 'user' || m.isMeta || m.toolUseResult || m.isCompactSummary)
    return undefined
  const originKind = (m.origin as { kind?: string } | undefined)?.kind
  if (originKind !== undefined && originKind !== 'human') return undefined
  ...
  switch (request.request.subtype) {
    ...
    default:
      ...
      const unknownSubtype =
        (request.request as { subtype?: string }).subtype ?? 'unknown'
      response = {
        type: 'control_response',
        response: {
          subtype: 'error',
          request_id: request.request_id,
          error: `REPL bridge does not handle control_request subtype: ${unknownSubtype}`,
        },
      }
  }
