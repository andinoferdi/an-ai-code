...
export function CompanionSprite(): React.ReactNode {
  const reaction: AppState['companionReaction'] = useAppState((s: AppState) => s.companionReaction)
  ...
  const [petStart, setPetStart] = useState<{
    petStartTick: number
    forPetAt: AppState['companionPetAt']
  }>({ petStartTick: 0, forPetAt: petAt })
  ...
  const petAge = petAt ? tick - petStart.petStartTick : Infinity
...
export function CompanionFloatingBubble(): React.ReactNode {
  ...
  const [bubbleState, setBubbleState] = useState<{
    tick: number
    forReaction: AppState['companionReaction']
  }>({ tick: 0, forReaction: reaction })

  if (reaction !== bubbleState.forReaction) {
    setBubbleState({ tick: 0, forReaction: reaction })
  }
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setBubbleState(
        (s: { tick: number; forReaction: AppState['companionReaction'] }) => ({
          ...s,
          tick: s.tick + 1,
        }),
      )
  ...
  return (
    <SpeechBubble
      ...
      fading={bubbleState.tick >= BUBBLE_SHOW - FADE_WINDOW}
      tail="down"
    />
  )
}
